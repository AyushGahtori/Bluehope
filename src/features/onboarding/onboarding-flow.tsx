...
function isSelfServeAccountRole(value: unknown): value is SelfServeAccountRole {
  return (
    value === "customer" ||
    value === "soleProvider" ||
    value === "institution"
...
export function OnboardingFlow({ role }: { role: Role }) {
  ...
  const establishingRoleRef = useRef(false);
  const roleConflictRef = useRef(false);
  ...
  const desiredAccountRole = accountRoleFor[role];
  ...
  const checkProviderProfile = async (): Promise<
    | { kind: "complete" }
    | { kind: "incomplete"; profile: ExistingProfile }
    | { kind: "unknown" }
  > => {
  ...
  const applyProfileToSteps = (profile: ExistingProfile) => {
    if (role === "provider") {
      ...
      setOrgDetails((current) => ({
        ...
        registration:
          current.registration || profile.details?.registrationNumber || "",
      }));
    }
  ...
  const reflectAuthUser = (user: FirebaseUser) => {
    setAuthUser({
      uid: user.uid,
  ...
  const rememberAuthUser = (
    user: FirebaseUser,
    establishedRole: SelfServeAccountRole,
  ) => {
    writeStoredAuthUser({
      uid: user.uid,
    ...
    reflectAuthUser(user);
  };

  const resolveCurrentAccount = async (
    user: FirebaseUser,
  ): Promise<{
    role: SelfServeAccountRole | null;
    email: string | null;
    ...
    try {
      ...
      return {
        role: isSelfServeAccountRole(data.role) ? data.role : null,
        email:
  ...
  const showRoleConflict = (
    existingRole: SelfServeAccountRole,
    email?: string | null,
  ) => {
    roleConflictRef.current = true;
    clearStoredAuthUser();
    setAuthUser(null);
    setRoleConflict({ existingRole, email: email ?? null });
    setAuthMessage("");
    setCheckingAccount(false);
    setStep(signInStep);
  };

  const resetForDifferentAccount = () => {
    establishingRoleRef.current = false;
    roleConflictRef.current = false;
  ...
  useEffect(() => {
    roleConflictRef.current = Boolean(roleConflict);
  }, [roleConflict]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (establishingRoleRef.current || roleConflictRef.current) {
        reflectAuthUser(user);
        return;
      }

      const account = await resolveCurrentAccount(user);
      if (cancelled) return;
      ...
      if (account.role !== desiredAccountRole) {
        showRoleConflict(account.role, account.email);
        return;
      }

      rememberAuthUser(user, account.role);

      const completed =
        account.onboardingCompleted ||
        (await isParentOnboardingComplete(user.uid));
      ...
      if (!cancelled) {
        ...
        setBasicInfo((current) => ({
          ...
          firstName:
            current.firstName || (user.displayName ?? "").split(" ")[0] || "",
          lastName:
            current.lastName ||
  ...
  }, [role, router, desiredAccountRole]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      setStep((current) =>
        current < signInStep + 1 ? signInStep + 1 : current
      );
    });
    ...
    return pool
      .filter((condition) =>
        [
          ...
          ...condition.subcategories.map((item) => item.name),
        ]
          .join(" ")
          .toLowerCase()
          .includes(query),
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      establishingRoleRef.current = true;
      roleConflictRef.current = false;
      setRoleConflict(null);
      setCheckingAccount(true);
      setAuthMessage("Opening Google sign-in...");
      const outcome = await signInWithGoogleAndEstablishRole(
        desiredAccountRole,
      );

      if (outcome.kind === "conflict") {
        showRoleConflict(
          outcome.existingRole,
          outcome.email ?? authUser?.email ?? null,
        );
        return;
      }
      ...
      if (
        role === "parent" &&
        (await isParentOnboardingComplete(outcome.uid))
      ) {
        router.replace("/dashboard/parent");
        return;
      }
      ...
      setBasicInfo((current) => ({
        ...
        firstName:
          current.firstName || (outcome.displayName ?? "").split(" ")[0] || "",
        lastName:
          current.lastName ||
          (outcome.displayName ?? "").split(" ").slice(1).join(" "),
      }));
  ...
  const validateCurrentStep = () => {
    ...
    if (role === "parent" && step === 2) {
      ...
      if (!/^[0-9+\-\s()]{8,}$/.test(basicInfo.phone))
                ...
                <div key={item} className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600">
                      Built around calm, privacy-aware discovery.
                    </p>
                  </div>
                </div>
              ))}
                ...
                >
                  {roleConflict ? (
                    <RoleConflictScreen
                      ...
                      onUseDifferentAccount={resetForDifferentAccount}
                    />
                  ) : role === "parent" ? (
                    <ParentStep
                      step={step}
    ...
    return (
      <div className="space-y-5">
        <h3 className="text-xl font-bold">
          What condition or support need are you looking for help with?
        </h3>
        <p className="mt-2 text-slate-600">
          Select multiple options. This is for discovery, not diagnosis.
        </p>
    ...
    <div className="mx-auto max-w-md py-5">
      <div className="rounded-[16px] border border-blue-100 bg-white p-7 text-center shadow-soft">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Continue with Google to securely connect this account to BlueHope.
        ...
        {authMessage ? (
          <p
            className={cn(
              ...
              authUser ? "text-emerald-700" : "text-bluehope",
            )}
          >
            {authMessage}
          </p>
...
function ProviderStep({
    ...
    return (
      <div className="grid gap-5 sm:grid-cols-2">
        <Input
          ...
          onChange={(event) =>
            onDetailsChange({ firstName: event.target.value })
          }
          placeholder="First name"
        />
        <Input
          ...
          onChange={(event) =>
            onDetailsChange({ lastName: event.target.value })
          }
          placeholder="Last name"
        />
        <Input
          ...
          onChange={(event) =>
            onDetailsChange({ displayName: event.target.value })
          }
  ...
  if (step === 4) {
    return (
      <OpeningHoursStep value={weeklyHours} onChange={onWeeklyHoursChange} />
    );
  }
    ...
    return (
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <Input
          ...
          onChange={(event) =>
            onDetailsChange({ representative: event.target.value })
          }
        ...
        <Input
          ...
          onChange={(event) =>
            onDetailsChange({ designation: event.target.value })
          }
    ...
    return (
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <Input
          ...
          onChange={(event) =>
            onDetailsChange({ registration: event.target.value })
          }
        ...
        <Input
          ...
          onChange={(event) =>
            onDetailsChange({ foundedYear: event.target.value })
          }
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-600">
          These are operating hours. Appointment slots are managed separately
          from the dashboard.
        </p>
      </div>
      <div className="grid gap-3">
        {DAY_KEYS.map((day) => {
          ...
          return (
            <div
              key={day}
              className="grid gap-3 rounded-[12px] border border-slate-200 p-4 sm:grid-cols-[140px_1fr_auto] sm:items-center"
            >
              <p className="font-bold text-slate-950">{DAY_LABELS[day]}</p>
              {closed ? (
                <p className="rounded-[8px] bg-slate-100 px-4 py-3 text-sm font-semibold text-slate-500">
                  Closed
                </p>
              ) : (
                <div className="grid gap-3 sm:grid-cols-2">
                  <Input
                    ...
                    onChange={(event) =>
                      setDay(day, { ...hours, open: event.target.value })
                    }
                  />
                  <Input
                    type="time"
                    ...
                    onChange={(event) =>
                      setDay(day, { ...hours, close: event.target.value })
                    }
                  />
                </div>
              )}
    ...
    <div className="space-y-5">
      ...
      <div className="bluehope-enter bluehope-lift rounded-[8px] border border-dashed border-bluehope bg-blue-50 p-8 text-center">
        ...
        <p className="mt-1 text-sm text-slate-600">
          Allowed files: JPG, PNG, PDF. Private by default.
        </p>
      </div>
    </div>
  );
...
function LocationStep({
  ...
  const requestLocation = () => {
    ...
    if (!navigator.geolocation) {
      setStatus("error");
      onLocationTextChange?.(
        "Location is not available in this browser. Enter your area manually below.",
      );
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          onLocationTextChange?.(
            readable
              ? `Current location captured: ${readable}`
      ...
      <div className="flex min-h-[300px] flex-col items-center justify-center rounded-[12px] bg-soft-blue p-8 text-center">
        ...
        {locationText ? (
          <motion.p
            ...
            className={cn(
              "mt-4 text-sm font-medium",
              status === "done" ? "text-emerald-700" : "text-rose-600",
            )}
          >
            {locationText}
          </motion.p>
        ...
        <div className="mt-5 w-full max-w-sm">
          ...
          <p className="mt-2 text-xs text-slate-500">
            Manually entered areas are stored as user-provided information, not
      ...
      {provider ? (
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          {["In-clinic", "At-home", "Online"].map((mode) => (
            <label
              key={mode}
              className="flex gap-3 rounded-[8px] border border-slate-200 p-4"
            >
              <input
                type="checkbox"
                defaultChecked
                className="h-4 w-4 accent-bluehope"
              />
              {mode}
            </label>
          ))}
